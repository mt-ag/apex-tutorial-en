# 17. Application Search: Create Search Regions and Configurations

In this task, you will create a search region and configurations that seek results in multiple data sources. You will need the knowledge described in Chapter 13 for this purpose.

The goal is to provide your users with a powerful search engine-style feature that searches multiple data sources for results in minutes and displays them in a single unified search result region.

## 17.1. Create a Search Configuration
...
The following two examples illustrate the search using local data and with a REST data source.
...
- Select the following values, then click **Create Search Configuration**:
...
- Click on the created search configuration **Product_Search** to adjust the settings.

![](../../assets/Chapter-17/search_config_06.jpg)  
...
- Under **Icon and Display**, select the following values:
  ...
  A **No Results Found Message** can be added:
...
Using the RESTful service and REST data source created in Chapter 13, we will create a search configuration that is based on the REST data source **Departments**.
...
- Select the following values, then click **Next**:
...
## 17.2. Use Search Configurations in the Application  

To use the search configurations, create a page where the search results are displayed and a search field where you can enter the search term.

### 17.2.1. Create Search Page 
...
## 17.3. Incorporate Search Field into the Navigation Bar  
...
- The first action is of type *Execute JavaScript Code* with the following settings:
...
- The second action is of type *Execute Server-side Code* with the following settings:
...
- And the third action is again of type *Execute JavaScript Code* with the following settings:
...
- Enter **Search Page Item** as **P0_SEARCH** and then click **Save**.
...
The search field now appears in the navigation bar, is available on every page, and redirects you to the search results page when a search value is entered and the enter key is pressed.
