...
**Template Components** are a new type of plug-in in APEX. They allow you to define an HTML template (with or without additional CSS and JavaScript) and use placeholders. They are much easier to use than a full region plug-in, where you do not need deep knowledge of the plug-in APIs.

On each page, you can then create an area of this plug-in type, place a query, and then get an instance of that template filled with the data for each returned row. You can also render a single instance in a region or even use them in columns of interactive reports.
...
6. In the next step, scroll down to the **Custom Attributes** section and delete all existing attributes and click on **Synchronize from Templates**.
...
10. The **CSS file** will be created with the following name.
...
11. Copy the **CSS code** below and paste it into APEX. Note the **Reference** link at the end, as it will be needed shortly.
...
12. Insert the copied **Reference** link at the appropriate place as shown in the screenshot and save.
...
15. Create a new region on the page with the title: **Progress Bars**. Then select the previously created plugin: **Progress Bars** as the type.
...
18. Finally, a button will be created here and the APEX page saved.
