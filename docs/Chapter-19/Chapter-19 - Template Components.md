...
**Template Components** are a new plug-in type in APEX. They allow you to define an HTML template (with or without additional CSS and JavaScript) using placeholders. They are much easier to use than a full region plug-in, as you don't need in-depth knowledge of the plug-in APIs.

On each page, you can create a section of this plug-in type, place a query, and then obtain an instance of this template filled with data for each returned row. You can also render a single instance in a region or even use it in columns of interactive reports.

## <a name="erstellung-eines-template-components-apex-plugin"></a>19.1 Creating a "Template Components" (APEX Plugin)

1. Open **Shared Components**, click on **Plug-ins** and then on **create**
...
4. In the next step, insert the following code in **Partial**, **Report Body**, and **Report Row** at the corresponding locations as per the screenshot. Finally, press the **Create Plug-in** button.
...
6. In the next step, scroll down to the **Custom Attributes** section, delete all existing attributes, and click on **Synchronize from Templates**.

![](../../assets/Chapter-19/6.jpg)

7. Now there should be only 3 attributes visible. Click on the first attribute **Color Index**
...
11. Copy the **CSS Code** below and insert it into APEX. Note the **Reference** link at the end, as we will need it shortly.
...
12. Insert the copied **Reference** link as shown in the screenshot at the appropriate place and save.

![](../../assets/Chapter-19/12.jpg)

Up to this step, the **Plug-in** has been successfully created.

13. In the next step, a new APEX page with the plugin will be created.
...
16. Then select **SQL Query** as the type and insert the SQL code below into the **SQL-Query**. Then switch to the **Attributes** tab.
...
19. Finally, the page should look like this. Press the Refresh Button to reload **random** values.
